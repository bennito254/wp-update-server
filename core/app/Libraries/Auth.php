...
class Auth
{
	...
	public function __construct()
	{
		...
		$this->ionAuthModel = new \App\Models\AuthModel();
		...
		if ($user)
		{
			...
			if ($code)
			{
				...
				else
				{
                    ...
                    $subject = "Recover Your Password - ".get_option('site_title', 'Trading');
                    $email = $user->email;
                    $mailer = new Mailer();
//                  $this->email->clear();
...
//					if ($this->email->send())
		...
		else
		{
			...
			else
			{
				$emailSent = $this->sendEmail(
					$email,
					"Activate Account - ".get_option('site_title', 'Trading'),
		...
		array $data): bool
	{
        ...
        $mailer = new Mailer();
        if ($mailer->sendEmail($subject, $message, $user_email)) {
            return TRUE;
        }
	...
	public function logout(): bool
	{
		...
		$this->session->remove([$identity, 'id', 'user_id', 'email', 'identity', 'old_last_login', 'last_check']);
