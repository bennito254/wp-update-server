 ...
 * Class AuthModel
 *
 * @property AuthModel $ion_auth The Ion_auth library
 */
class AuthModel
{
	/**
	...
	public function deactivate(int $id=0): bool
	{
		...
		else if ((new \App\Libraries\Auth())->loggedIn() && $this->user()->row()->id == $id)
		{
	...
	public function login(string $identity, string $password, bool $remember=false): bool
	{
		...
		$query = $this->db->table($this->tables['users'])
						  ->select($this->identityColumn . ', email, id, password, active, last_login, two_factor, two_factor_secret')
						  ->where($this->identityColumn, $identity)
						  ->limit(1)
						  ->orderBy('id', 'desc')
						  ->get();
		...
		if (isset($user))
		{
			if ($this->verifyPassword($password, $user->password, $identity))
			{
				...
				if (!$this->config->allow2FA || $user->two_factor == '0') {

                    return $this->completeLogin($user, $password, $remember, $identity);
                } else {
                    session()->set('_2fa_user', $user);
                    session()->set('_2fa_identity', $identity);
                    session()->set('_2fa_password', $password);
                    session()->set('_2fa_remember', $remember);
                    session()->set('_2fa_required', true);
    ...
    public function complete2FALogin()
    {
        $user = session()->get('_2fa_user');
        $identity = session()->get('_2fa_identity');
        $password = session()->get('_2fa_password');
        $remember = session()->get('_2fa_remember');
        $required = session()->get('_2fa_required');

        if ($this->completeLogin($user, $password, $remember, $identity)) {
            session()->remove(['_2fa_user', '_2fa_identity', '_2fa_password', '_2fa_remember', '_2fa_required']);
            return TRUE;
        }
    ...
    public function completeLogin($user, $password, $remember, $identity)
    {
	 ...
	 * @author aditya menon
	 */
	public function createGroup(string $groupName='', string $groupDescription='', array $additionalData=[]): bool|int
    {
