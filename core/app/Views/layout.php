...
<head>
    ...
    <meta content="Wordpress Plugin and Themes self-hosted updates server" name="description"/>
            ...
            <div class="container-fluid">
                ...
                <div class="brand">
                    <a href="<?php echo site_url(); ?>" class="logo">
                        <span>
                            <img src="<?php echo assets_url('images/logo-sm.png'); ?>"
                                 style="max-height: 30px" alt="logo-small" class="logo-sm">
                        </span>
                    </a>
                </div>
                <div class="collapse navbar-collapse" id="navbarTop">
                    <ul class="navbar-nav mb-2 mb-lg-0">
                        ...
                        <li class="nav-item">
                            <a class="nav-link" href="<?php echo route('admin.access_logs') ?>">Access Logs</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" href="<?php echo route('admin.settings') ?>">Settings</a>
                        </li>
                    </ul>
                </div>
                <ul class="topbar-item list-unstyled d-inline-flex align-items-center mb-0">
                    ...
                    <li class="dropdown topbar-item">
                        ...
                        <div class="dropdown-menu dropdown-menu-end py-0">
                            ...
                            <a class="dropdown-item" href="<?php echo route('admin.account.settings') ?>"><i
                                        class="las la-cog fs-18 me-1 align-text-bottom"></i>Account Settings</a>
                            <a class="dropdown-item" href="<?php echo route('admin.account.security') ?>"><i
                                        class="las la-lock fs-18 me-1 align-text-bottom"></i> Security</a>
                            <div class="dropdown-divider mb-0"></div>
                            <a class="dropdown-item text-danger" href="<?php echo route('auth.logout') ?>"><i
...
if ($error) {
    if (is_array($error)) {
        foreach ($error as $it) {
            ?>
            <script>
                toast('Error', "<?php echo $it ?>", 'danger');
            </script>
            <?php
        }
    } else {
        ?>
        <script>
            toast('Error', "<?php echo $error ?>", 'danger');
        </script>
        <?php
    }
}
if ($success) {
    ?>
    <script>
        toast('Success', "<?php echo $success ?>", 'success');
    </script>
    <?php
}
if ($messages) {
    if (is_array($messages)) {
        foreach ($messages as $message) {
            ?>
            <script>
                toast('Info', "<?php echo $message ?>", 'info');
            </script>
            <?php
        }
    } else {
        ?>
        <script>
            toast('Info', "<?php echo $messages ?>", 'info');
        </script>
        <?php
    }
}
